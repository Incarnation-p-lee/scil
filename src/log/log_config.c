static inline void
log_config_option_set_tail(char *option)
{
    char *c;
...
static inline char *
log_config_option_space_skip(char *option)
{
    char *c;
...
static inline void
log_config_option_set(char *option)
{
    assert_exit(option);

    option = log_config_option_space_skip(option);
    ...
    log_config_option_set_tail(option);
...
static inline void
log_config_option_parse(FILE *fd)
{
    assert_exit(fd);
    ...
    while (dp_strchr(config_buf, SCIL_CONFIG_MASK) != NULL) {
        log_config_option_set(config_buf);
...
log_config_initial(void)
{
    ...
    if (!config) {
        ...
        log_config_option_parse(config);
